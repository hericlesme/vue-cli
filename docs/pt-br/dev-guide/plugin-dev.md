...
Um plugin CLI é um pacote npm que pode adicionar recursos adicionais ao projeto usando o Vue CLI. Esses recursos podem incluir:

- alterações na configuração do webpack do projeto - por exemplo, você pode adicionar uma nova regra de resolução do Webpack para uma determinada extensão de arquivo, se o seu plug-in funcionar com esse tipo de arquivo. Digamos que `@ vue / cli-plugin-typescript` adicione essa regra para resolver as extensões` .ts` e `.tsx`;
- adição de um novo comando vue-cli-service - por exemplo, `@vue/cli-plugin-unit-jest` adiciona um novo comando `test: unit` que permite ao desenvolvedor rodar testes unitários;
- extensão do `package.json` - uma opção útil quando o seu plugin adiciona algumas dependências ao projeto e você precisa adicioná-las à seção de dependências do pacote;
- criação de novos arquivos no projeto e/ou modificação os antigos. Às vezes é uma boa ideia criar um componente de exemplo ou modificar um arquivo principal para adicionar algumas importações;
- solicitar que o usuário selecione certas opções - por exemplo, você pode perguntar ao usuário se deseja criar o componente de exemplo mencionado acima.

:::tip
Não use em demais plugins vue-cli! Se você quiser apenas incluir uma certa dependência, por exemplo [Lodash](https://lodash.com/) - é mais fácil fazê-lo manualmente com npm do que criar um plug-in específico apenas para fazê-lo.
:::

O Plugin CLI deve sempre conter um [Service Plugin](#service-plugin) como sua exportação principal, e pode opcionalmente conter um [Generator](#generator), um [Prompt File](#prompts) e um [Vue UI integration](#ui-integrtion).

Como um pacote npm, o plugin CLI deve ter um arquivo `package.json`. Também é recomendado ter uma descrição do plugin no `README.md` para ajudar outras pessoas a encontrarem seu plugin no npm.

Portanto, a estrutura típica de pastas de plug-ins da CLI é semelhante à seguinte:
...
Uma parte Generator do plug-in CLI geralmente é necessária quando você deseja estender seu pacote com novas dependências, criar novos arquivos em seu projeto ou editar os existentes.

Dentro do plugin da CLI, o generator deve ser colocado em um arquivo `generator.js` ou `generator/index.js`. Será invocado em dois cenários possíveis:

- Durante a criação inicial do projeto, se o plug-in da CLI estiver instalado como parte da predefinição de criação do projeto.

- Quando o plugin é instalado após a criação do projeto e invocado individualmente via `vue add` ou `vue invoke`.

Um generator deve exportar uma função que recebe três argumentos:

1. Uma instância [GeneratorAPI](generator-api.md);

2. As opções do generator para este plugin. Estas opções são resolvidas durante a fase [prompt](#prompts) da criação do projeto, ou carregadas a partir de uma predefinição salva em `~/.vuerc`. Por exemplo, se o `~/.vuerc` salvo se parece com isto:
...
Para um plugin de terceiros, as opções serão resolvidas a partir dos prompts ou argumentos de linha de comando quando o usuário executar o `vue invoke` (consulte [Prompts para plug-ins de terceiros](#prompts-para-plugins-de-terceiros)).

3. Toda a predefinição (`presets.foo`) será passada como o terceiro argumento.

### Criando novos modelos

Quando você chama `api.render ('./template')`, o gerador renderizará os arquivos em `./template` (resolvidos em relação ao arquivo gerador) com [EJS](https://github.com/mde/ejs).

Vamos imaginar que estamos criando um plugin [vue-cli-plugin-auto-routing](https://github.com/ktsn/vue-cli-plugin-auto-routing) e queremos fazer as seguintes alterações no projeto no plugin invocar:

- crie uma pasta `layouts` com um arquivo de layout padrão;
- crie uma pasta `pages` com as páginas` about` e `home`;
- adicione um `router.js` à raiz da pasta` src`

Para renderizar esta estrutura, você precisa criá-la primeiro dentro da pasta `generator / template`:

! [Estrutura do gerador] (/ generator-template.png)

Depois que o template é criado, você deve adicionar a chamada `api.render` ao arquivo` generator / index.js`:
